...
# Сборка и установка в пользовательскую директорию
build-install: build ## Собрать и установить в пользовательскую директорию
	@echo "$(YELLOW)Установка CLI в пользовательскую директорию...$(NC)"
	@mkdir -p ~/.local/bin
	@cp $(BUILD_DIR)/$(BINARY_NAME) ~/.local/bin/
	@echo "$(GREEN)✅ CLI установлен в ~/.local/bin/$(NC)"
	@echo "$(YELLOW)💡 Для использования добавьте ~/.local/bin в PATH:$(NC)"
	@echo "   echo 'export PATH=\$$HOME/.local/bin:\$$PATH' >> ~/.bashrc"
	@echo "   echo 'export PATH=\$$HOME/.local/bin:\$$PATH' >> ~/.zshrc"
	@echo "$(GREEN)🎉 Теперь можно использовать: ai-agents-cli --help$(NC)"
...
install-user: build ## Установить CLI в пользовательскую директорию
	@echo "$(YELLOW)Установка CLI в пользовательскую директорию...$(NC)"
	@mkdir -p ~/.local/bin
	@cp $(BUILD_DIR)/$(BINARY_NAME) ~/.local/bin/
	@echo "$(GREEN)✅ CLI установлен в ~/.local/bin/$(NC)"
	@echo "$(YELLOW)Добавьте ~/.local/bin в PATH:$(NC)"
	@echo "   echo 'export PATH=\$$HOME/.local/bin:\$$PATH' >> ~/.bashrc"
	@echo "   echo 'export PATH=\$$HOME/.local/bin:\$$PATH' >> ~/.zshrc"

# Полная настройка CLI
setup: build-install ## Собрать, установить и настроить CLI
	@echo "$(YELLOW)Настройка CLI...$(NC)"
	@./scripts/setup.sh

# Установка автодополнения
install-completion: ## Установить автодополнение для текущей оболочки
	@echo "$(YELLOW)Установка автодополнения...$(NC)"
	@./scripts/install-completion.sh

# Установка автодополнения для Fish
install-fish-completion: ## Установить автодополнение для Fish shell
	@echo "$(YELLOW)Установка автодополнения для Fish...$(NC)"
