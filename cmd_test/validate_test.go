...
func init() {
	...
	os.Setenv("IAM_KEY_ID", "test-key-id")
	os.Setenv("IAM_SECRET", "test-secret")
...
`
	...
	// Создаем невалидный конфигурационный файл (YAML)
...
// Вспомогательные функции из cmd/validate.go
...
func loadSchemas(validator *validator.ConfigValidator) error {
	...
	for name, path := range schemas {
		if err := validator.LoadSchema(name, path); err != nil {
			// Игнорируем ошибки загрузки схем в тестах
			continue
		}
	}
